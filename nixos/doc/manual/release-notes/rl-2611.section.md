...
- `matrix-conduit` now uses rocksdb as the default database backend when `system.stateVersion` is set to `"26.11"` or later. A manual migration is necessary if you used sqlite as the backend before.
